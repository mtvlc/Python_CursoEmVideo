...
def Application(self, master=None): # application module
    ...
    def Main():
        ...
        self.cta = Label(self.top, height=3, font=('', 12, 'bold'), text='Insira três números abaixo\npara ver o maior e o menor.') # call to action
    #    if len(self.numbers) == 0: # check if numbers list is empty
        self.cta.pack(pady=(5, 10)) # if true show call to action
        if len(self.numbers) < 3:
            self.numberLabel = Label(self.top, font=('', 8, 'bold'), text='{}º número.'.format(len(self.numbers) + 1))
            ...
            self.numbers.sort()
            tkinter.messagebox.showinfo('Result', 'Maior número: {}\nMenor número: {}'.format(self.numbers[-1], self.numbers[0]))
            self.numbers.clear()
            self.top.destroy()
            Main()
